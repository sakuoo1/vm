...
class VMTPathRenamer(QWidget):
    ...
    def init_ui(self):
        ...
        self.addon_window_btn = styled_button("🧩 Collecteur Addon")
    ...
    def _check_update(self):
        url = "https://raw.githubusercontent.com/sakuoo1/vm/main/test.py"
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if local_code.strip() != remote_code.strip():
                    self.update_label.setText("❌ Application pas à jours")
                    self.progress.setValue(50)
                    with open(local_path, "w", encoding="utf-8") as f:
                        f.write(remote_code)
                    self.progress.setValue(100)
                    self.update_label.setText("✅ Mise à jour appliquée, relance...")
                    QApplication.processEvents()
                    os.execv(sys.executable, ['python'] + sys.argv)
                else:
                    self.update_label.setText("✅ Application à jours")
                    self.progress.setValue(100)
            else:
